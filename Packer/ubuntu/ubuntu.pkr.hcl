...
############################
# Input variables (inherited from variables.pkr.hcl)
############################
...
############################
# Builder – create new VM via ISO
############################

source "proxmox-iso" "ubuntu" {
  proxmox_url              = var.proxmox_url
  proxmox_api_token_id     = var.proxmox_api-token_id
  ...
  vm_name         = "ubuntu-template-{{timestamp}}"
  template_description = "Automated Ubuntu Server template via Packer"
...
build {
  ...
  # Cleanup cloud-init, machine-id, logs, etc. (good for templating)
  provisioner "shell" {
    inline = [
