#!/bin/bash
# Prepare Debian 13 template build
...
echo -e "${BLUE}=== Preparing Debian 13 Packer Build ===${NC}"
...
if [ ! -f "$TEMP_ISO" ]; then
  echo -e "${BLUE}Downloading Debian 13 ISO...${NC}"
  wget -q --show-progress \
    "http://ftp.us.debian.org/debian-cd/current/amd64/iso-cd/${DEBIAN_ISO}" \
    -O "$TEMP_ISO"
else
...
echo ""
echo -e "${BLUE}Ready to run: packer build -var-file=\"../credentials.pkvars.hcl\" -var-file=\"../variables.pkvars.hcl\" debian-13.pkr.hcl${NC}"
