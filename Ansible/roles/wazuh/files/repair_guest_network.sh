...
guest_gateway="$4"
guest_nameservers="$5"
guest_search_domain="${6:-local.sinlessgames.com}"
...
resolv_lines+=("search ${guest_search_domain}")
...
iface ${iface} inet static
    ...
    dns-search ${guest_search_domain}
EOF
)"
...
guest_exec "chattr -i /etc/resolv.conf || true; rm -f /etc/resolv.conf || true"
