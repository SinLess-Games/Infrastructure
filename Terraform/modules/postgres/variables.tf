# =====================================================================
# Postgres Cluster Terraform Module Variables
# =====================================================================
...
variable "cluster_name" {
  description = "Name of the Postgres cluster (used for naming resources)"
  type        = string
  default     = "postgres-ha"
}
...
variable "target_nodes" {
  description = "List of Proxmox node names for distributing Postgres VMs"
  type        = list(string)
  validation {
...
variable "vm_id_start" {
  description = "Starting VM ID for Postgres nodes (e.g., 206)"
  type        = number
  default     = 206
}
...
variable "cpu_cores" {
  description = "Number of CPU cores per Postgres node"
  type        = number
  default     = 4
}

variable "cpu_sockets" {
  description = "Number of CPU sockets per Postgres node"
  type        = number
  default     = 1
}

variable "memory_mb" {
  description = "Memory allocation per Postgres node (MB)"
  type        = number
  default     = 8192
}

variable "disk_size" {
  description = "Disk size for each Postgres node (e.g., 80G)"
  type        = string
  default     = "80G"
}
...
variable "vlan_id" {
  description = "VLAN ID for Postgres cluster"
  type        = number
  default     = 20
}
...
variable "gateway" {
  description = "Network gateway for Postgres nodes"
  type        = string
}
...
variable "ip_addresses" {
  description = "List of IP addresses for Postgres nodes (must match node_count)"
  type        = list(string)
  validation {
    condition     = length(var.ip_addresses) >= 3
    error_message = "At least 3 IP addresses must be provided for Postgres HA"
  }
}
...
variable "backup_enabled" {
  description = "Enable Proxmox backup for Postgres VMs"
  type        = bool
  default     = true
}
...
variable "ha_enabled" {
  description = "Enable Proxmox HA for Postgres VMs"
  type        = bool
  default     = true
}

variable "ha_group" {
  description = "Proxmox HA resource group for Postgres VMs"
  type        = string
  default     = "postgres-ha"
}
...
variable "tags" {
  description = "Tags to apply to Postgres VMs"
  type        = list(string)
  default     = ["postgres", "patroni", "ha"]
}

variable "description" {
  description = "Description for Postgres VMs"
  type        = string
  default     = "PostgreSQL HA cluster node"
}
